...
pub fn parse_github_repo_url(input: &str) -> Option<ParsedRepositoryUrl> {
	...
	if trimmed.starts_with("git@github.com:") {
		...
		return Some(ParsedRepositoryUrl { owner, repo, r#ref: None });
	}

	let after = trimmed.splitn(2, "github.com/").nth(1)?;
	let mut parts: Vec<&str> = after.split('/').collect();
	if parts.len() < 2 { return None; }

	if let Some(pos) = parts.last().and_then(|s| s.find(['?', '#'])) {
		if let Some(last) = parts.last_mut() { *last = &last[..pos]; }
	}

	let owner = parts[0];
	let repo_raw = parts[1];
	if owner.is_empty() || repo_raw.is_empty() { return None; }
	...
	Some(ParsedRepositoryUrl { owner: owner.to_string(), repo: repo.to_string(), r#ref: reference })
}
